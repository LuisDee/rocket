...
- **Micro:** concrete sessions for a rolling 7–10 days, placed into availability slots (work + commute + swim evenings + Immerse lesson + Baker markers). Regenerated weekly and on any replan trigger.
...
- Swim sessions coexist with easy runs on the same day; never with quality + long combined.
...
- Run-commute slots (Colindale→City ~15km, or partial) are the preferred vehicle for easy volume **if enabled** in availability — biggest lever for fitting 60km around five swim evenings.
