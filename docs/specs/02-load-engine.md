...
- **Input ranges** (`READINESS.inputScales`, PROVISIONAL): yesterday-RPE 1-10, soreness 0-5, sleep 0-9 hours, motivation 1-5. Soreness alone starts at zero because "none" is a real reading, while the 1-5 severity in `sorenessBlocksQuality` describes soreness that exists. These were implied by the weights for as long as the weights existed and stated nowhere, which made every one of them a magic number at the call site; the sleep mark in particular is a guess at what a full night is for this athlete rather than a measurement.
