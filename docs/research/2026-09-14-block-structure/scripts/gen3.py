...
V1_MIN = 1.4
MLR_FRAC, MLR_MIN, MLR_MAX, MLR_MIN_WEEK = 0.65, 18, 24, 70
...
SLOTS = [  # id, weekdays(ISO), maxKm, startLocal
    ...
    ("evening", {1, 2, 3, 4, 5, 6, 7}, 25, "18:30"),  # Luis 15 Sep: evenings up to 25 km
...
def plan_week(week, prev_hard, quality_dates_global):
    ...
    for rd, _, _, eff in RACES:
        # Lincoln is 33 km with 21.1 at marathon pace: its first run after is recovery too, not only raced efforts
        if eff not in ("raced", "marathon-pace"): continue
...
def size(week, dates, roles, rest, added_rest, qdate, hard, dist, next_hard, order, notes):
    ...
    for d in run_days:
        ...
        if r in ("recovery", "recovery-first"): km[d] = min(REC_DEFAULT, cap[d])
            ...
            km[d] = min(q, cap[d])
    mlr = None
    if medium and target >= MLR_MIN_WEEK and lkm:
        mlr = medium
        km[medium] = min(max(r05(MLR_FRAC * lkm), MLR_MIN), MLR_MAX, cap[medium])
    in_target = sum(km[d] for d in dates if not (d in RACE_ON and d != (D(ldate) if ldate else None)))
    R = target - in_target
    # capped proportional fill
    active = [d for d in ga if d != mlr]
    ...
    for d in [x for x in ga if x != mlr] + ([medium] if medium else []) + [d for d in run_days if roles.get(d, "").startswith("recovery")]:
    ...
    if mlr: notes.append(f"medium-long run {km[mlr]} km on {mlr}")
